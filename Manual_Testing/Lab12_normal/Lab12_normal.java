...
public class Lab12_normal{
	public static sequence fastestreach(int[] nonreachableCities, int sourceCity, int destinationCity, int[] visitedArray, airport[] listOfCities, int presentTime, String flightID, int count, int initialDeparture){
		if(sourceCity == destinationCity){
			...
			sequence minimumData = new sequence();
			sequence travelData;
			if(nonreachableCities[sourceCity] <= presentTime){
				return minimumData;
			}
			...
			int travelTime;
			while(temp!=null){
				if(presentTime <= temp.flightDetails.departure && visitedArray[temp.flightDetails.destination]!=1){
					travelData = fastestreach(nonreachableCities, temp.flightDetails.destination, destinationCity, visitedArray, listOfCities, temp.flightDetails.arrival, temp.flightDetails.id, count+1, initialDeparture);
			...
			if(minimumData.time == 2400){
				nonreachableCities[sourceCity] = presentTime;
			}
			return minimumData;
		}
	}
	...
	public static void main(String[] args) throws IOException{
		...
		sequence travelData;
		int travelTime;
		int[] nonreachableCities = new int[502];
		for(int i=1; i<=N; i++){
			nonreachableCities[i] = 2400;
		}
		while(temp!=null){	
			travelData = fastestreach(nonreachableCities, temp.flightDetails.destination, destinationCity, visitedArray, listOfCities, temp.flightDetails.arrival, temp.flightDetails.id, 1, temp.flightDetails.departure);
