...
public void dijkstra(Node src,Node des,String t1,String t2,Node[] parent,int[] dist,minheap mh,Node[] zZ,int n){
	int V=v;
	/*int[] dist=new int[V];
	...
	while(mh.isempty()==false){
		...
		while(pcrawl != null){
			...
			if(u.arr.equals("0") && Integer.parseInt(pcrawl.arr)<=Integer.parseInt(t2) && Integer.parseInt(pcrawl.dep)>=Integer.parseInt(t1)&& u.weight[pcrawl.f1.id-n] >= 0){
				int vq=pcrawl.f1.id;
				...
				int vq=pcrawl.f1.id;
				if(mh.isinminheap(vq)==true && dist[u.f1.id] != Integer.MAX_VALUE && u.weight[V-n]+pcrawl.dur+dist[u.f1.id] < dist[vq] ){
					dist[vq]=dist[u.f1.id]+u.weight[V-n]+pcrawl.dur;
...
public class Lab12 {
	...
	public static void main(String[] args) {
		...
		try{//long startTime = System.currentTimeMillis();
			...
			//g.display();
			g.dijkstra(z[src-1],z[des-1],t1,t2,parent,dist,mh,z,n);
