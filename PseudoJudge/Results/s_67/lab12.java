...
public class lab12 {
	...
	//	System.out.println(C1+" "+C2+" "+D+" "+A);
		for (int i=0;i<num;i++)
		{
			...
			Q[++q]=new1;
		/*	if(new1.f.equalsIgnoreCase("VETQB"))
			{
				flight ptr=new1;
...
/*		for(int i=0;i<=q;i++)
	...
	/*	for(int i=0;i<x;i++)
		...
		for(int i=0;i<x;i++)
		{
			if(arr[i][C2-1]!=null)
			{
				if(arr[i][C2-1].sum==999999999)
					continue;
				
				flight ptr=arr[i][C2-1];
				int nm=0;
				while(ptr!=null)
				{
					nm++;
	...
	public void display(flight root)
	{
		if(root==null)
			return;
		else
